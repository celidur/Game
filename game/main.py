...
async def loading_animation(x, y):
    ...
    n = 0
    while not ready:
        Game.Screen.blit(pygame.image.load("game/assets/temp/loading_background.png"), (0, 0))
        ...
        Game.Screen.blit(font_.render('.' * (n // 3), False, (255, 255, 255)),
                         (352 + font_.size(loading_text)[0] // 2, 670))
        for i in range(12):
            Game.Screen.blit(pygame.image.load("game/assets/icons/loading/{}.png".format((i + n) % 12)),
...
def runnig_game():
    global running, onclick, pos, temp
    while not ready:
